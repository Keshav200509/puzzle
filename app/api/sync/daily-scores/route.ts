...
import { isBetterRun, normalizeDailyEntry, type DailySyncEntry, validateDailyEntry } from '@/lib/server/validation';

type Body = { entries?: DailySyncEntry[] };
...
export async function POST(request: Request) {
  const route = '/api/sync/daily-scores';
  const method = 'POST';
  ...
  try {
    ...
    const rate = checkRateLimit(`daily:${ipKey}`);
    if (!rate.allowed) {
    ...
    for (const entry of entries) {
      const reason = validateDailyEntry(entry);
    ...
    const normalized = entries.map(normalizeDailyEntry);
    ...
    await prisma.$transaction(async (tx) => {
      for (const entry of normalized) {
        const existing = await tx.dailyScore.findUnique({
          where: { userId_date: { userId: user.id, date: entry.date } }
        });
